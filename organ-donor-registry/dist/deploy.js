...
import { Contract } from './managed/organ_donor/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'organ_donor');
...
const PrivateStateId = 'organDonorPrivateState';
// ---------------------------------------------------------------------------
...
async function main() {
    ...
    console.log('\n╔══════════════════════════════════════════════════════════╗');
    console.log('║        Organ Donor Registry Deploy Script       ║');
    ...
    // --- Compile contract ---
    const counterCompiledContract = CompiledContract.make('organ_donor', Contract).pipe(CompiledContract.withVacantWitnesses, CompiledContract.withCompiledFileAssets(zkConfigPath));
    // --- Deploy ---
    const contract = await withStatus('Deploying organ donor contract (this may take a few minutes)', async () => {
